...
def generate_wordlist(base, max_len=3, digits="0123456789", max_words=10000):
    """
    Simple combinatoric generator: appends 0..n digits to base and common suffixes.
    Limited to max_words to avoid explosion. Educational only.
    """
    ...
    for suf in suffixes:
        ...
        if len(out) >= max_words:
            break
    # also add numeric permutations up to max_len
    for l in range(1, max_len+1):
        for i in range(min(100, len(digits)**l)):
            # generate some pseudo-random numeric combinations (not exhaustive)
...
# --- Main menu ---
def main_menu():
    print("=== Brute-force Simulator (Educational, Local Only) ===")
    while True:
        print("\nMenu:")
